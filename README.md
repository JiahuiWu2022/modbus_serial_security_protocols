...
three prototype management-plane backends are illustrative instantiations. Just for reference, any deployment related to the application scenario, may define or replace the management-plane credential protocol as long as it authenticates the transcript, derives SAC/content keys with labeled separation, and satisfies the APDU state, replay, fragmentation, and fail-closed invariants. 
...
If a hardware UART interface is used, the socket initialized here is only a placeholder and is not used for data transfer. Every frame is read from and sent through UART.
